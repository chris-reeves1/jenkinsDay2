            ...
            parallel {
                ...
                stage("image build metadata") {
                    ...
                    post {
                        always {
                            archiveArtifacts(
                                artifacts: "docker_inspect.json,docker_history.txt,docker_image.txt",
                                allowEmptyArchive: true
                            )
                        }
                    }
                ...
                stage("trivy image scan") {
                    steps {
                        sh """
                            trivy image \
                                --cache-dir /tmp/trivycache-image \
                                --format json \
                ...
                stage("generate SBOM") {
                    ...
                    post {
                        always {
                            archiveArtifacts(
                                artifacts: "flask-app-sbom.cdx.json",
                                allowEmptyArchive: true
                            )
                        }
                    }
        ...
        stage("size gate") {
            steps {
                script {
                    ...
                    echo "Image size: ${sizeBytes} bytes"
